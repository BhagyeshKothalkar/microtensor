...
Tensor sum_to_shape(const Tensor& input, const std::vector<size_t>& target) {
  ...
  Tensor out = broadcast_to_shape(Tensor::zeros(aligned), input.shape());
  TensorIterator<float, const float>(out, input)
...
Tensor sum(const Tensor& a, const std::vector<index_t>& dims) {
  ...
  Tensor kept = broadcast_to_shape(Tensor::zeros(keep), a.shape());
  TensorIterator<float, const float>(kept, a).for_each(
