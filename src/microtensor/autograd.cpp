...
void Tensor::add_grad(const Tensor& g) const {
  ...
  Tensor effective_g =
      (g.shape() == shape_) ? g : reduce_sum_to(g, shape_);

  if (!autograd_meta_->grad_) {
    autograd_meta_->grad_ =
        std::make_unique<Tensor>(effective_g.clone());
  } else {
...
void Tensor::backward() const {
  ...
  if (!autograd_meta_ || !autograd_meta_->grad_) {
    ...
    autograd_meta_->grad_ =
        std::make_unique<Tensor>(Tensor::ones(shape_));
  }
  ...
  std::function<void(IGradNode*)> build_topo =
      [&](IGradNode* node) {
